...
The `filter_uuid.py` script helps you select Akri-discovered devices for firmware updates. Install Python 3.x and the `kubernetes` and `pyyaml` packages, then run the script with `python3 filter_uuid.py`. Follow prompts to filter by device type, application type, or UUID, select devices interactively, specify the firmware image, and configure batch sizes and delays for gradual rollouts. The script generates a FlashJob CR, which the Operator applies, as detailed in the [tutorial](/docs/tutorials/filter_uuid.md).
